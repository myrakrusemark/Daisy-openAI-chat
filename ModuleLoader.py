...
class ModuleLoader:
    def __init__(self, directory=""):
        ...
        self.Daisy_wake_instances = []
        globals()["Daisy_wake_instances"] = self.Daisy_wake_instances
        #>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        for root, dirs, files in os.walk(self.directory):
            for filename in files:
                if filename.endswith('.py'):
                    ...
                    module_name = "modules."+rel_path[:-3].replace(os.sep, ".")  # remove .py extension and replace slashes with dots
                    try:
